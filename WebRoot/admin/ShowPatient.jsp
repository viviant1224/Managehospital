...
if(pageBean!=null) {
patientlist = (List<Patient>)pageBean.getList();
} else {
patientlist = (List<Patient>)request.getAttribute("patientlist");
}
...
<script type="text/javascript">
	function chuyuan(pid) {
		str = window.showModalDialog('chuyuanZhuanjia.action?pid='+pid+'&'+ Math.random(), '', 'dialogWidth:800px;dialogHeight:700px');
		if (str == 'OK') {
				winReload();
		}
	}
			...
			<div class="span12">
				<div class="box">
									<div class="box-head tabs">
										...
										<li class='active'>
											<form action="searPaSearch.action" class="navbar-form navbar-left" role="search">
									        <input type="text" name="searchPa" class="form-control" placeholder="Search">
...
<table class="table table-responsive" width="900px" align="center">
  <tr class="success">
  <td width="80px"><b>医保卡号</b></td>
  <td width="120px"><b>头像</b></td>
  <td width="50px"><b>姓名</b></td>
  <td width="100px"><b>密码</b></td>
  <td width="100px"><b>电话</b></td>
  <td width="50px"><b>生日</b></td>
  <td width="80px"><b>账户余额</b></td>
  <td width="120px"><b>住址</b></td>
  <td width="100px"><b>当前状态</b></td>
  ...
  if(patientlist.size()!=0&&patientlist!=null) {
  	...
  	%>
  	<tr class="success">
  <td width="80px"><b><%=patientlist.get(i).getId() %></b></td>
   <td width="120px"><a href="<%=path%>/upload/<%=patientlist.get(i).getPimageFileName()%>" class='preview fancy'><img src="<%=path%>/upload/<%=patientlist.get(i).getPimageFileName()%>" height="80px" width="80px" /></td>
  <td width="50px"><b><%=patientlist.get(i).getPname() %></b></td>
  <td width="100px"><b><%=patientlist.get(i).getPassword()%></b></td>
  <td width="100px"><b><%=patientlist.get(i).getPtel()%></b></td>
  <td width="50px"><b><%=patientlist.get(i).getPbirthday() %></b></td>
  <td width="80px"><b><%=patientlist.get(i).getPmoney()%></b></td>
  <td width="120px"><b><%=patientlist.get(i).getPaddress()%></td>
  <td width="100px"><b><%=patientlist.get(i).getPstate()%></td>
 
  <td width="200px">
  <%
  	if(patientlist.get(i).getPstate().equals("就诊中")) {
  	%>
  	<button class="btn btn-success" data-toggle="modal" data-target="#myModal<%=patientlist.get(i).getId() %>">
		  出院
	</button>
  	<%
  	} else if(patientlist.get(i).getPstate().equals("待定")){
  	%>
  	<button class="btn btn-info" disabled>
 		待定
	</button>
  	<%
  	} else if(patientlist.get(i).getPstate().equals("预约中")){
  	%>
   ...
   %>
    ...
    <!-- Modal 出院的模态窗口-->
<div class="modal fade" id="myModal<%=patientlist.get(i).getId() %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="myModalLabel">病人出院</h4>
      	</div>
      <div class="modal-body">
     确定姓名为：<%=patientlist.get(i).getPname() %>编号为：<%=patientlist.get(i).getId() %>的病人出院？
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onClick="chuyuan('<%=patientlist.get(i).getId()%>')">确定</button>
...
<li><s:a href="showpaUserinfoDoc.action?pageBean.firstPage=0&pageBean.pageButton=0">首页 </s:a></li>
  	
	<s:if test="%{#request.pageBean.pageNum!=1}">
	<li><a href="showpaUserinfoDoc.action?pageBean.firstPage=<s:property value="pageBean.firstPage"/>&pageBean.pageButton=1">上一页  </a></li>
	</s:if>
	
	
	<s:if test="%{#request.pageBean.maxPageNum!=#request.pageBean.pageNum}">
	<li><a href="showpaUserinfoDoc.action?pageBean.firstPage=<s:property value="pageBean.firstPage"/>&pageBean.pageButton=2">下一页  </a></li>
	</s:if>
	
	
<li><a href="showpaUserinfoDoc.action?pageBean.firstPage=<s:property value="pageBean.firstPage"/>&pageBean.pageButton=3">尾 页</a></li>
